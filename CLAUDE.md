...
### 업무분야 9개 카테고리 ↔ 페이지 (모두 `px-*` 디자인 통일 완료)
nav 드롭다운/홈 카테고리 그리드의 순서(`ncat.1`~`ncat.9`)와 파일 매핑. 링크는 `site.js`의 `NCAT_HREF` 배열로 관리(한 곳만 고치면 데스크톱·모바일 동시 반영). 홈 그리드 링크는 `index.html`에 하드코딩.
1. 민사·형사 → [civil-criminal.html](civil-criminal.html)
2. 이혼·행정소송 → [divorce-admin.html](divorce-admin.html)
3. 교통사고·손해배상 → [traffic-accident.html](traffic-accident.html)
4. 산업재해·체불임금 → [industrial-accident.html](industrial-accident.html) (기준 템플릿)
5. 불법체류·출국명령 → [immigration-overstay.html](immigration-overstay.html)
6. 체류비자·초청·통역 → [visa-invitation.html](visa-invitation.html)
7. 난민 신청 → [refugee.html](refugee.html)
8. 행정심판(음주·영업정지) → [admin-appeal.html](admin-appeal.html)
9. 인허가·법인설립 → [licensing-corporate.html](licensing-corporate.html)

- 신뢰 지표 4번째는 전 페이지 공통 "0원 / 1차 상담 무료"(산재만 "6종 급여" 유지).
- `practice.html`은 9개 카테고리 허브 — 본문 `.cat-grid`가 위 9개 페이지로 링크(라벨은 `ncat.*`라 자동 다국어). (litigation/cta 섹션은 옛 디자인 잔존, 추후 정리 가능)
...
- 페이지별 번역 파일: `js/i18n-<slug>.js` (slug = cc/da/ta/ia/io/vi/rf/aa/lc). 각 파일이 11개 언어 사전을 `window.I18N`에 `Object.assign`으로 병합. 로드 순서: `i18n.js` → `i18n-<slug>.js` → `site.js`. (en/vi/ru/zh/uz/th/km/si/ne/mn 번역은 AI 생성 — 게시 전 검수 권장)
